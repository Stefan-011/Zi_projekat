...
namespace ZI_projekat
{
    ...
    internal class Tiger_hash
    {
        ...
        public void Round(ref ulong a, ref ulong b, ref ulong c, int x_index, int mul)
        {
            c = c ^ x[x_index];
            byte[] Cbytes = UlongToBytes(c);
            a -= (T1[Cbytes[0]] ^ T2[Cbytes[2]] ^ T3[Cbytes[4]] ^ T4[Cbytes[6]]);
            b += (T4[Cbytes[1]] ^ T3[Cbytes[3]] ^ T2[Cbytes[5]] ^ T1[Cbytes[7]]);
            b *= (ulong)mul;
        }
        ...
        public void HashMsgBlk(byte[] msgblk)
        {
            blockHashed++;
        ...
        { 
            ...
            {
                byte[] messageBlock = new byte[msgBlkSizeByte];
                Array.Copy(array, i * msgBlkSizeByte, messageBlock, 0, msgBlkSizeByte);
                HashMsgBlk(messageBlock);
            }
            ...
            if (numOfFinalBytes > 0)
            {
                ...
                Array.Copy(array, numOfMessageBlocks * msgBlkSizeByte, finalBytes, 0, numOfFinalBytes);
                HashFinalMsgBlk(finalBytes, numOfFinalBytes, numOfBytes);
            }


            byte[] registerA = UlongToBytes(this.A);
            byte[] registerB = UlongToBytes(this.B);
            byte[] registerC = UlongToBytes(this.C);

            byte[] result = registerA;
            AppendBytes(ref result, registerB);
